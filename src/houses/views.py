...
from .forms import HouseForm, SectionInlineFormSet, ImageForm, StoreyInlineFormSet, HousesAdminInlineFormSet
...
class CreateHouse(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = self.get_form()
        imageform = ImageForm(data=request.POST, files=request.FILES)
        inlines = SectionInlineFormSet(request.POST, prefix='sections')
        inlines_2 = StoreyInlineFormSet(request.POST, prefix='storey')
        inlines_3 = HousesAdminInlineFormSet(request, prefix='worker')

        if form.is_valid() and imageform.is_valid() and inlines.is_valid() and inlines_2.is_valid() and inlines_3.is_valid(): # imageform.is_valid()
            return self.form_valid(form, imageform, inlines, inlines_2, inlines_3)
        else:
            return self.form_invalid(form, imageform, inlines, inlines_2, inlines_3)


    def form_valid(self, form, imageform, inlines, inlines_2, inlines_3):
        print("а це працює")
        ...
        for img in img_list:
            field_name = img
            img_obj = imageform.cleaned_data.get(img)
            if img_obj:
    ...
    def form_invalid(self, form, imageform, inlines, inlines_2, inlines_3):
        print(form.errors)
        print(imageform.errors)
        print(inlines.errors)
        print(inlines_2.errors)
        print(inlines_3.errors)
        return self.render_to_response(self.get_context_data(form=form, imageform=imageform, inlines=inlines,
...
class UpdateHouse(UpdateView):
   # template_name = 'update_house.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = self.get_form()
        imageform = ImageForm(data=request.POST, instance=self.object, files=request.FILES)
